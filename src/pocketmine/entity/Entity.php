...
abstract class Entity extends Location implements Metadatable, EntityIds
{
    ...
    public const DATA_COLOUR = 3; //byte
    ...
    public const DATA_FLAG_IMMOBILE = 16;
    public const DATA_FLAG_NO_AI = 16;
    ...
    public static function createBaseNBT(Vector3 $pos, ?Vector3 $motion = null, float $yaw = 0.0, float $pitch = 0.0): CompoundTag
    {
        return new CompoundTag('', [
            ...
            new ListTag('Motion', [
                new DoubleTag('', null !== $motion ? $motion->x : 0.0),
                new DoubleTag('', null !== $motion ? $motion->y : 0.0),
                new DoubleTag('', null !== $motion ? $motion->z : 0.0),
            ]),
    ...
    public function __construct(Level $level, CompoundTag $nbt)
    {
        ...
        $this->onGround = 0 !== $this->namedtag->getByte('OnGround', 0);
        $this->invulnerable = 0 !== $this->namedtag->getByte('Invulnerable', 0);
    ...
    {
        return 1 === $this->propertyManager->getByte(self::DATA_ALWAYS_SHOW_NAMETAG);
    }
    ...
    protected function initEntity(): void
    {
        if ($this->namedtag->hasTag('CustomName', StringTag::class)) {
            ...
            if ($this->namedtag->hasTag('CustomNameVisible', StringTag::class)) {
                ...
                $this->setNameTagVisible(0 !== $this->namedtag->getByte('CustomNameVisible', 1));
            }
        }
    }
    ...
    protected function doOnFireTick(int $tickDiff = 1): bool
    {
        ...
        if ((0 === $this->fireTicks % 20) or $tickDiff > 20) {
            $this->dealFireDamage();
        }
    ...
    {
        if (0 === count($this->level->getCollisionCubes($this, $this->getBoundingBox(), false))) {
            return false;
        }
        ...
        if (BlockFactory::$solid[$this->level->getBlockIdAt($floorX, $floorY, $floorZ)]) {
            ...
            if (Vector3::SIDE_WEST === $direction) {
                $this->motion->x = -$force;

                return true;
            }

            if (Vector3::SIDE_EAST === $direction) {
                $this->motion->x = $force;

                return true;
            }

            if (Vector3::SIDE_DOWN === $direction) {
                $this->motion->y = -$force;

                return true;
            }

            if (Vector3::SIDE_UP === $direction) {
                $this->motion->y = $force;

                return true;
            }

            if (Vector3::SIDE_NORTH === $direction) {
                $this->motion->z = -$force;

                return true;
            }

            if (Vector3::SIDE_SOUTH === $direction) {
                $this->motion->z = $force;
    ...
    public function onUpdate(int $currentTick): bool
    {
        ...
        if ($this->hasMovementUpdate()) {
            ...
            if (0 != $this->motion->x or 0 != $this->motion->y or 0 != $this->motion->z) {
    ...
    public function setPosition(Vector3 $pos): bool
    {
        ...
        if ($pos instanceof Position and null !== $pos->level and $pos->level !== $this->level) {
    ...
    protected function checkChunks(): void
    {
        ...
        if (null === $this->chunk or ($this->chunk->getX() !== $chunkX or $this->chunk->getZ() !== $chunkZ)) {
            if (null !== $this->chunk) {
    ...
    {
        if (null === $this->chunk or $this->closed) {
            return;
        }
