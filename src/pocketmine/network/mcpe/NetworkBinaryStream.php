...
class NetworkBinaryStream extends BinaryStream
{
    ...
    public function getSlot(): Item
    {
        ...
        if (null !== $nbt) {
            if ($nbt->hasTag(self::DAMAGE_TAG, IntTag::class)) {
                ...
                if (null !== ($conflicted = $nbt->getTag(self::DAMAGE_TAG_CONFLICT_RESOLUTION))) {
    ...
    public function putSlot(Item $item): void
    {
        ...
        if ($item instanceof Durable and $item->getDamage() > 0) {
            if (null !== $nbt) {
                if (null !== ($existing = $nbt->getTag(self::DAMAGE_TAG))) {
                    $nbt->removeTag(self::DAMAGE_TAG);
    ...
    protected function getCommandOriginData(): CommandOriginData
    {
        ...
        if (CommandOriginData::ORIGIN_DEV_CONSOLE === $result->type or CommandOriginData::ORIGIN_TEST === $result->type) {
    ...
    protected function putCommandOriginData(CommandOriginData $data): void
    {
        ...
        if (CommandOriginData::ORIGIN_DEV_CONSOLE === $data->type or CommandOriginData::ORIGIN_TEST === $data->type) {
