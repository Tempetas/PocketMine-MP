...
public static function isValidUserName(?string $name): bool
{
    ...
    return 'rcon' !== $lname and 'console' !== $lname and $len >= 1 and $len <= 16 and 0 === preg_match('/[^A-Za-z0-9_ ]/', $name);
}
...
public function sendCommandData()
{
    ...
    foreach ($this->server->getCommandMap()->getCommands() as $name => $command) {
        if (isset($pk->commandData[$command->getName()]) or 'help' === $command->getName() or !$command->testPermissionSilent($this)) {
            continue;
        }
...
{
    return null !== $this->sessionAdapter;
}
...
public function sendChunk(int $x, int $z, BatchPacket $payload)
{
    ...
    if (-1 !== $this->spawnChunkLoadCount and ++$this->spawnChunkLoadCount >= $this->spawnThreshold) {
...
{
    if (!$this->isConnected() or -1 === $this->viewDistance) {
        return;
    }
    ...
    for ($x = 0; $x < $radius; ++$x) {
        for ($z = 0; $z <= $x; ++$z) {
            ...
            /* Top right quadrant */
            if (!isset($this->usedChunks[$index = Level::chunkHash($centerX + $x, $centerZ + $z)]) or false === $this->usedChunks[$index]) {
                $newOrder[$index] = true;
            }
            unset($unloadChunks[$index]);

            /* Top left quadrant */
            if (!isset($this->usedChunks[$index = Level::chunkHash($centerX - $x - 1, $centerZ + $z)]) or false === $this->usedChunks[$index]) {
                $newOrder[$index] = true;
            }
            unset($unloadChunks[$index]);

            /* Bottom right quadrant */
            if (!isset($this->usedChunks[$index = Level::chunkHash($centerX + $x, $centerZ - $z - 1)]) or false === $this->usedChunks[$index]) {
                $newOrder[$index] = true;
            }
            unset($unloadChunks[$index]);

            /* Bottom left quadrant */
            if (!isset($this->usedChunks[$index = Level::chunkHash($centerX - $x - 1, $centerZ - $z - 1)]) or false === $this->usedChunks[$index]) {
                $newOrder[$index] = true;
            }
            unset($unloadChunks[$index]);
            ...
            if ($x !== $z) {
                ...
                if (!isset($this->usedChunks[$index = Level::chunkHash($centerX + $z, $centerZ + $x)]) or false === $this->usedChunks[$index]) {
                    $newOrder[$index] = true;
                }
                unset($unloadChunks[$index]);

                /* Top left quadrant mirror */
                if (!isset($this->usedChunks[$index = Level::chunkHash($centerX - $z - 1, $centerZ + $x)]) or false === $this->usedChunks[$index]) {
                    $newOrder[$index] = true;
                }
                unset($unloadChunks[$index]);

                /* Bottom right quadrant mirror */
                if (!isset($this->usedChunks[$index = Level::chunkHash($centerX + $z, $centerZ - $x - 1)]) or false === $this->usedChunks[$index]) {
                    $newOrder[$index] = true;
                }
                unset($unloadChunks[$index]);

                /* Bottom left quadrant mirror */
                if (!isset($this->usedChunks[$index = Level::chunkHash($centerX - $z - 1, $centerZ - $x - 1)]) or false === $this->usedChunks[$index]) {
                    $newOrder[$index] = true;
                }
                unset($unloadChunks[$index]);
            }
...
{
    return null !== $this->spawnPosition and $this->spawnPosition->isValid();
}
...
{
    if ($literal) {
        return Player::SURVIVAL === $this->gamemode;
    } else {
        return 0 === ($this->gamemode & 0x01);
    }
}
...
{
    if ($literal) {
        return Player::CREATIVE === $this->gamemode;
    } else {
        return 1 === ($this->gamemode & 0x01);
    }
}
...
{
    if ($literal) {
        return Player::ADVENTURE === $this->gamemode;
    } else {
...
{
    return Player::SPECTATOR === $this->gamemode;
}
...
protected function handleMovement(Vector3 $newPos): void
{
    ...
    if (!$revert and 0 != $distanceSquared) {
        $dx = $newPos->x - $this->x;
        $dy = $newPos->y - $this->y;
        $dz = $newPos->z - $this->z;
...
public function checkNetwork()
{
    ...
    if (PHP_INT_MAX !== $this->nextChunkOrderRun and $this->nextChunkOrderRun-- <= 0) {
...
public function handleLogin(LoginPacket $packet): bool
{
    ...
    if (($this->isBanned() or $this->server->getIPBans()->isBanned($this->getAddress())) and
...
public function onVerifyCompleted(LoginPacket $packet, ?string $error, bool $signedByMojang): void
{
    ...
    $this->processLogin($packet->protocol);
}

    /**
     * @return void
     */
protected function processLogin($ver)
{
    ...
    if (null === ($level = $this->server->getLevelByName($this->namedtag->getString('Level', '', true)))) {
        $this->setLevel($this->server->getDefaultLevel());
    ...
    foreach ($achievements as $achievement) {
        $this->achievements[$achievement->getName()] = 0 !== $achievement->getValue();
    }
    ...
    $pk = new ResourcePacksInfoPacket($ver);
...
public function handleMovePlayer(MovePlayerPacket $packet): bool
{
    ...
    if (null !== $this->forceMoveSync and $newPos->distanceSquared($this->forceMoveSync) > 1) {  //Tolerate up to 1 block to avoid problems with client-sided physics when spawning in blocks
...
public function handleEntityEvent(ActorEventPacket $packet): bool
{
    if ($packet->entityRuntimeId !== $this->id) {
        ...
        return ActorEventPacket::EATING_ITEM === $packet->event;
    }
    if (!$this->spawned or !$this->isAlive()) {
        return true;
    }
...
public function handleInventoryTransaction(InventoryTransactionPacket $packet): bool
{
    ...
    foreach ($packet->actions as $networkInventoryAction) {
        if (NetworkInventoryAction::SOURCE_TODO === $networkInventoryAction->sourceType and (
                NetworkInventoryAction::SOURCE_TYPE_CRAFTING_RESULT === $networkInventoryAction->windowId or
                NetworkInventoryAction::SOURCE_TYPE_CRAFTING_USE_INGREDIENT === $networkInventoryAction->windowId
            ) or (
                null !== $this->craftingTransaction &&
                !$networkInventoryAction->oldItem->equalsExact($networkInventoryAction->newItem) &&
                NetworkInventoryAction::SOURCE_CONTAINER === $networkInventoryAction->sourceType &&
                ContainerIds::UI === $networkInventoryAction->windowId &&
                UIInventorySlotOffset::CREATED_ITEM_OUTPUT === $networkInventoryAction->inventorySlot
            )
        ) {
            $isCraftingPart = true;
        }
    ...
    if ($isCraftingPart) {
        if (null === $this->craftingTransaction) {
    ...
    } elseif (null !== $this->craftingTransaction) {
        ...
        case InventoryTransactionPacket::TYPE_USE_ITEM:
            ...
            switch ($type) {
                case InventoryTransactionPacket::USE_ITEM_ACTION_CLICK_BLOCK:
                    ...
                    $spamBug = (null !== $this->lastRightClickData and
                        ...
                        $this->lastRightClickData->clickPos->distanceSquared($packet->trData->clickPos) < 0.00001
                    );
...
public function handleInteract(InteractPacket $packet): bool
{
    ...
    if (InteractPacket::ACTION_MOUSEOVER === $packet->action and 0 === $packet->target) {
...
{
    if (!$this->spawned or (!$this->isAlive() and PlayerActionPacket::ACTION_RESPAWN !== $packet->action)) {
        return true;
    }
...
{
    if (!$this->isAlive() && RespawnPacket::CLIENT_READY_TO_SPAWN === $packet->respawnState) {
...
public function handleBookEdit(BookEditPacket $packet): bool
{
    ...
    if (Item::WRITABLE_BOOK !== $oldBook->getId()) {
        return false;
    }
...
public function sendDataPacket(DataPacket $packet, bool $needACK = false, bool $immediate = false)
{
    ...
    try {
        ...
        if ($needACK and null !== $identifier) {
...
public function attack(EntityDamageEvent $source): void
{
    ...
    if ($this->isCreative()
        and EntityDamageEvent::CAUSE_SUICIDE !== $source->getCause()
        and EntityDamageEvent::CAUSE_VOID !== $source->getCause()
    ) {
        $source->setCancelled();
    } elseif ($this->allowFlight and EntityDamageEvent::CAUSE_FALL === $source->getCause()) {
        $source->setCancelled();
    }
...
{
    if ($this->spawned and null === $players) {
...
{
    if (ContainerIds::NONE !== ($id = $this->getWindowId($inventory))) {
        return $id;
    }
...
public function removeWindow(Inventory $inventory, bool $force = false)
{
    ...
    if (null !== $id and !$force and isset($this->permanentWindows[$id])) {
...
class Player extends Human implements CommandSender, ChunkLoader, IPlayer
{
    ...
    public function handleInventoryTransaction(InventoryTransactionPacket $packet) : bool
    {
        ...
        $isCraftingPart = false;
        foreach ($packet->actions as $networkInventoryAction) {
            if ($networkInventoryAction->sourceType === NetworkInventoryAction::SOURCE_TODO and (
