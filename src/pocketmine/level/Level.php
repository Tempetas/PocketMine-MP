...
class Level implements ChunkManager, Metadatable
{
    ...
    private $lightUpdateReduction = 0;
    ...
    public function unload(bool $force = false): bool
    {
        ...
        foreach ($this->getPlayers() as $player) {
            if ($this === $defaultLevel or null === $defaultLevel) {
        ...
        if (count($this->changedBlocks) > 0) {
            if (count($this->players) > 0) {
                foreach ($this->changedBlocks as $index => $blocks) {
                    if (0 === count($blocks)) { //blocks can be set normally and then later re-set with direct send
                        continue;
                    }
    ...
    {
        if ($this->chunksPerTick <= 0 or 0 === count($this->loaders)) {
            $this->chunkTickList = [];
            ...
            foreach ($chunk->getSubChunks() as $Y => $subChunk) {
                if (!($subChunk instanceof EmptySubChunk)) {
                    ...
                    for ($i = 0; $i < 3; ++$i) {
                        ...
                        if (null !== $this->randomTickBlocks[$blockId]) {
                            /** @var Block $block */
    ...
    {
        if (!$this->isInWorld($pos->x, $pos->y, $pos->z) or
    ...
    public function isFullBlock(Vector3 $pos): bool
    {
        ...
        return null !== $bb and $bb->getAverageEdgeLength() >= 1;
    }
    ...
    public function getBlockAt(int $x, int $y, int $z, bool $cached = true, bool $addToCache = true): Block
    {
        ...
        if ($addToCache and null !== $relativeBlockHash) {
    ...
    {
        if (16 != $this->lightUpdateReduction) {
            ++$this->lightUpdateReduction;

            return;
        }

        $this->lightUpdateReduction = 0;
    ...
    public function useBreakOn(Vector3 $vector, Item &$item = null, Player $player = null, bool $createParticles = false): bool
    {
        ...
        $drops = [];
        if (null === $player or !$player->isCreative()) {
        ...
        $xpDrop = 0;
        if (null !== $player and !$player->isCreative()) {
    ...
    public function useItemOn(Vector3 $vector, Item &$item, int $face, Vector3 $clickVector = null, Player $player = null, bool $playSound = false): bool
    {
        ...
        if (Block::AIR === $blockClicked->getId()) {
            return false;
        }
    ...
    public function getCollidingEntities(AxisAlignedBB $bb, Entity $entity = null): array
    {
        ...
        if (null === $entity or $entity->canCollide) {
            ...
            for ($x = $minX; $x <= $maxX; ++$x) {
                for ($z = $minZ; $z <= $maxZ; ++$z) {
                    foreach ($this->getChunkEntities($x, $z) as $ent) {
                        ...
                        if ($ent->canBeCollidedWith() and (null === $entity or ($ent !== $entity and $entity->canCollideWith($ent))) and $ent->boundingBox->intersectsWith($bb)) {
                            $nearby[] = $ent;
                        }
                    }
                }
    ...
    {
        return null !== ($chunk = $this->getChunk($X, $Z)) ? $chunk->getEntities() : [];
    }
    ...
    {
        return null !== ($chunk = $this->getChunk($X, $Z)) ? $chunk->getTiles() : [];
    }
    ...
    public function generateChunkCallback(int $x, int $z, ?Chunk $chunk)
    {
        ...
        if (isset($this->chunkPopulationQueue[$index = Level::chunkHash($x, $z)])) {
            ...
            if (null !== $chunk) {
                ...
                if ((null === $oldChunk or !$oldChunk->isPopulated()) and $chunk->isPopulated()) {
    ...
    public function setChunk(int $chunkX, int $chunkZ, Chunk $chunk = null, bool $deleteEntitiesAndTiles = true)
    {
        ...
        if (null !== $oldChunk and $oldChunk !== $chunk) {
    ...
    public function isChunkGenerated(int $x, int $z): bool
    {
        ...
        return null !== $chunk ? $chunk->isGenerated() : false;
    }
    ...
    public function isChunkPopulated(int $x, int $z): bool
    {
        ...
        return null !== $chunk ? $chunk->isPopulated() : false;
    }
    ...
    public function getSafeSpawn(?Vector3 $spawn = null): Position
    {
        ...
        $x = (int) $v->x;
        $z = (int) $v->z;
        if (null !== $chunk and $chunk->isGenerated()) {
            $y = (int) min($max - 2, $v->y);
            $wasAir = (0 === $chunk->getBlockId($x & 0x0f, $y - 1, $z & 0x0f));
            for (; $y > 0; --$y) {
